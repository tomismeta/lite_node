...
let create ~admitted ~target ~request ~model ~pins ~input =
       ...
       (match Inference_request.check ~target ~requirement request with
        ...
        | Ok () ->
          (match Inference_model.check ~target model with
           ...
           | Ok () ->
             if not (String.equal (Inference_store.model_root pins) target.model_root) then
               Error
                 (Pin_model_root_mismatch
                    (Inference_store.model_root pins, target.model_root))
             else if not (String.equal (Inference_store.store_root pins) target.store_root) then
               Error
               ...
               then
                 Error
                   (Pin_ranges_root_mismatch
                      (Inference_store.model_ranges_root pins, expected_ranges_root))
               else
                 let actual_input_root = sha256 input in
                 if not (String.equal actual_input_root request.input_root) then
                   Error (Input_root_mismatch (request.input_root, actual_input_root))
                 else if String.length input > requirement.limits.max_view_bytes then
                   Error
                     (Input_limit_exceeded
                        (String.length input, requirement.limits.max_view_bytes))
                 else
                   Ok { admitted; target; request; model; pins; input })))
