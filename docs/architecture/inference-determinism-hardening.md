...
| Inference proof surface | `LOAD_F32_LE_FP`, `LOAD_F64_LE_FP`, `LINEAR_Q1_G128_FP`, `SIGMOID_FP`, `SOFTPLUS_FP`, `SILU_FP`, `CAUSAL_DEPTHWISE_CONV1D_FP`, `GATED_DELTA_RULE_FP`, `RMSNORM_FP_EPS`, `L2NORM_FP`, `ELEMWISE_MUL_FP`, `RESIDUAL_ADD_FP`, `ROPE_APPLY_INDEXED_FP`, `ATTENTION_SCORES_FP`, `SOFTMAX_FP`, `ATTENTION_WEIGHTED_SUM_FP`, `ARGMAX_FP` | Selectively admitted by the inference harness under explicit capabilities. Spine-used profiles (`byte-ingress-exact` f64 load, `deterministic-q1-g128-fp64-linear`, `deterministic-fp64-normalization`, `deterministic-fp64-comparison`) are `consensus_ready` after multi-platform spine matrix (Darwin arm64 + Linux x86_64). Softmax/gated-delta remain `consensus_candidate`. Host-trig and host-exp activations remain local-only. |
