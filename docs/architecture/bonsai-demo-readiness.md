...
blocker is no longer repeated advance itself; ABI-v2 multi-transition bundles
must declare exactly one initial `prefill` followed by zero or more `decode`
transitions, with `decode_steps` bound to the declared decode count. Remaining
product blockers are committed state payload transport and decode token output
contract binding. Session reports now include the canonical transition output
payload and its SHA-256 next to `output_root`; a decode transition may declare a
`selected_index` `output_contract` so LiteNode can report a root-bound VM-emitted
